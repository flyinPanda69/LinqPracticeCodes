...
namespace Module_5
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var evenOrOddNumbers = arrayOfNumbers.OrderBy(y => y).GroupBy(n => (n % 2 == 0) ? "Even" : "Odd")
                                    .OrderBy(x => x.Count());
            ...
            var ageGroups = people.GroupBy(x => x.Age < 20
                                                        ? "Young" : x.Age > 23
                                                        ? "Old" : "Adult");
            ...
            //Improvised let - Expand the above syntax
            ...
            var howManyOfEachGroup = people.GroupBy(x => x.Gender).Select(g => new
            {
                Gender : g.Key,
                                            Count: g.Count()
                                        });
            ...
            {
                Console.WriteLine($"Gender : {item.Gender}, Count : {item.Count}");
            }
